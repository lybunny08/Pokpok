      ...
      <div className='grid grid-cols-2 md:grid-cols-5 gap-[16px] '>
        {images.map((src, idx) => (
          <img
            ...
            src={src}
            alt={`gallery-${idx}`}
            className={`h-[140px] md:h-[220px] w-full rounded-sm object-cover ${idx >= 4 ? 'hidden md:flex' : ''}`}
          />
        ))}
      </div>

      <Newsletters />
      <div className='flex flex-col md:flex-row gap-[24px] md:gap-0 justify-between'>
        ...
        <div className='flex flex-col gap-[80px] '>
          <div className='flex flex-col md:flex-row gap-[32px] pr-[40px] '>
